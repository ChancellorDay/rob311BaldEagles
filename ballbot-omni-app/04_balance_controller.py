...
from ps4Controller import ROB311BTController
...
Player1 = ROB311BTController(interface="/dev/input/js0")
rob311_bt_controller_thread = threading.Thread(target=Player1.listen, args=(10,))
...
if __name__ == "__main__":
    ...
    for t in SoftRealtimeLoop(dt=DT, report=True):
        ...
        totErrorX = totErrorX + error_x
        totErrorY = totErrorY + error_y
        ...
        derivX = (error_x - prevErrorX) / DT #THIS MAKES THIS TERM MASSIVE BECAUSE DT IS SO SMALL
        derivY = (error_y - prevErrorY) / DT

        # Proportional controller
        Tx = KP_THETA_X * error_x + (.6 * derivX + .4 *prevDerivX) * D_TERM_X #+ I_TERM_X * totErrorX # 
        Ty = KP_THETA_Y * error_y + (.6 * derivY + .4 *prevDerivY) * D_TERM_Y #+ I_TERM_Y * totErrorY # 

        prevErrorX = error_x
        prevErrorY = error_y

        Tz = Player1.tz_demo_1
        print("TZ:" + str(Tz))
